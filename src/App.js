...
const App extends Component {
    ...
    resetGame=()=>{
        this.setState({clickedImagesList:[], isGameInProgress:true})
    }
    ...
    renderScoreCard = ()=> {
        ...
        const isWon=clickedImagesList.length === imagesList.length
    ...
    clickImage = id => {
        const {imagesList} = this.state
        const {clickedImagesList}=this.state
        const isImagePresent = clickedImagesList.includes(id)
        const clickedImagesLength = clickedImagesList.length
        if(isImagePresent){
    ...
    renderImagesList=()=>{
        const shuffledImagesList=this.getShuffledImagesList()

        return(
            <ul className="images-list-container">
                {shuffledImagesList.map(imageObject =>(
                    <ImageCard
                      key={imageObject.id}
    ...
    render(){
        ...
        const {clickedImagesList, isGameInProgress, score} =this.state

        return(
            <div className="app-container">
                <NavBar 
                    currentScore ={clickedImagesList.length}
                    isGameInProgress={isGameInProgress}
                    score = {score}
                />
                <div className="image-game-body">
                    {isGameInProgress? this.renderImagesList(): this.renderScoreCard()}
                </div>
                <div className="image-container">
                    <img src={imageUrl} className="selected-image" alt="match"/>
                </div>
